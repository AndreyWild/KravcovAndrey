...
public class Hotel {
    ...
    public static void main(String[] args) {
        ...
        roomService.getAllOpenRooms(new RoomsPriceComparator()).forEach(System.out::println);

        System.out.println("---------Сортировка свободных комнат по вместимости--------------------------------------");
        roomService.getAllOpenRooms(new RoomsCapacityComparator()).forEach(System.out::println);

        System.out.println("---------Сортировка свободных комнат по количесту звезд----------------------------------");
        roomService.getAllOpenRooms(new RoomsStarsComparator()).forEach(System.out::println);
        ...
        roomService.getAllAvailableNumbersForDate(date).forEach(System.out::println);
        ...
        System.out.println("Стоимость номера + услуги: " + guestService.roomBill(2L));

        System.out.println("---------Посмотреть 3-х последних постояльцев номера и даты пребывания-------------------");
        roomService.showThreeLastGuestsRooms(5L);

        System.out.println("---------Посмотреть список услуг постояльца и их цену (сортировать по цене, по имени)----");
        guestService.getAllMaintenancesGuest(2L, new MaintPriceComparator()).forEach(System.out::println);
        System.out.println("*******************************************");
        ...
        multipleEntitiesService.PricesForMaintenancesAndRooms().forEach(System.out::println);
        System.out.println("*******************************************");
        multipleEntitiesService.PricesForMaintenancesAndRooms(new AbstrPiceComparator()).forEach(System.out::println);
